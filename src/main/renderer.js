...
function loadPrayers(){
  if (timezone != undefined && calPrayers != undefined){
    ...
    document.getElementById("duhrTime").innerText = window.api.getTimes(calPrayers.dhuhr,timezone, shortTimeFormat);
  ...
  if (lang == 'en'){
    ...
    if (prayers[0] != undefined){
        ...
        if (progressBar.style.width != '0%' && timeUntilCurrentPrayer[0] == -1 && timeUntilCurrentPrayer[1] >= -5){
          document.getElementById("timeLeft").innerText = playingAdhan
        }
            ...
            document.getElementById("timeLeft").innerText = timeUntil + prayers[3] + ": " + intToHour(time);
        }   
    } 
}
